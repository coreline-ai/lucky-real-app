...
MCP_PREFLIGHT_AUTH_TOKEN="$MCP_AUTH_TOKEN" npm --prefix mcp-server run preflight:playmcp
...
- `Authorization: Bearer` 정상 token 200.
- PlayMCP preflight의 `initialize → notifications/initialized → tools/list → 대표 tools/call` 흐름 통과.
- 404 endpoint, 405 protocol, 403 CORS, 413 size 분류 통과.
- `tools/list` 20개.
...
콘솔 실행 전에 임시 HTTPS URL이 로컬 preflight와도 호환되는지 확인한다.
